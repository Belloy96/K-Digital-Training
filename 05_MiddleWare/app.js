...
// 공식 문서에 따르면 요청 객체(req), 응답 객체(res),
// 애플리케이션의 요청-응답 주기 중 
// 그 다음의 미들웨어 함수에 대한 액세스 권한을 갖는 함수이며,
// 그 다음의 미들웨어는 일반적으로 next라는 이름의 변수로 표현
// 그 다음의 미들웨어는 요청과 응답의 중간(middle)에
// 위치하기 때문에 미들웨어라고 부름
// 요청과 응답을 조작해서 기능을 추가하기도 하고,
// 나쁜 요청을 걸러내는 역할을 함

// app.use(미들웨어) 의 형태로 사용
// 파라미터 자리에는 req, res, next인 함수를 넣어주면 되고,
// 위에서 아래로 순서대로 진행되며, 
...
app.use((req,res,next)=>{
    ...
    next(); // 다음 코드로 넘어가기 위해 안하면 안넘어감
});

app.get('/', (req, res,next)=>{
    console.log('실행됨!');
    next();
}, (req, res)=>{ // 여기에서 에러가 발생 (일부로 한거임)
    throw new Error('에러는 에러처리 미들웨어로 가지요');
}); 

// 파라미터는 총 4개!, err에는 에러에 대한 정보가 들어있음
// 에러 처리 미들웨어는 직접 연결하지 않아도,
// express가 에러를 기본적으로 처리하기는 하지만,
// 실무에서는 직접 해주는것이 좋음
// 에러 처리 미들웨어는 코드의 제일 아래에 위치하는 것이 좋음!
