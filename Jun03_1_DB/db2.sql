...
--															ex) TRUNC(123.456, -1)
--															ex) TRUNC(123.456, 0)
--		- MOD(m, n) 	: 숫자의 m을 n으로 나눈 나머지			ex) MOD(12, 10)
...
--		- LPAD(s, n ,c)			: 문자열 s의 왼쪽부터 지정한 자리수 n까지
--									지정한 문자 c로 채워줌
--												ex) LPAD('ABC', 5, '*')
--		- RPAD(s, n ,c)			: 문자열 s의 오른쪽부터 지정한 자리수 n까지
--									지정한 문자 c로 채워줌
--												ex) RPAD('ABC', 5, '*')
...
--		- SYSDATE : 현재 날짜 / 시간을 변환
--		- TO_DATE(s1, datetime형식) 	: 문자형 데이터를 날짜형으로 반환
select to_date ('2024-06-01 오전 06:00', 'YYYY-MM-DD AM HH:MI') from dual; 
-- 										AM이던 PM이던 줄 중 하나만 들어와도 구분해 줌
--		- TO_CHAR(date, datetim형식)	: 날짜형 데이터를 문자형으로 반환
...
-- 오늘 날짜 기준 시/분 조회 
--	(테이블 헤더에 각각 '시', '분' / 컬럼에 각각 '?시', '?분' 이라고 나오게)
-- 지금이 오전인지 오후인지 조회
...
-- 특수문자가 껴 있는 경우 " " 사용해서 별칭 만들었음

select to_char (to_date('2024-06-06 오전 07:23', 'YYYY-MM-DD AM HH:MI'), 'YYYY.MM.DD') from dual;
...
--		- MIN(컬럼명)			: 최소값
--		- SUM(컬럼명)			: 총합
--		- RANK(컬럼명) 		: 중복 순위만큼 다음 순위 값을 증가시킴 (ex. 1 2 3 3 5)
--		- DENSE_RANK(컬럼명) 	: 중복 순위가 존재해도 순차적으로 다음 순위값을 표시함 (ex. 1 2 3 3 4)
----------------------------------------------------------------------------------
-- 5. NULL 관련 함수
