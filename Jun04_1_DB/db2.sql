...
-- 데이터 조작어 (DML) 중에서 데이터를 넣게하는 명려어인 INSERT와
...
-- 연산자 시리즈 두번째 비교연산자(=, !=, >, <, >=, <=)
-- 연산자 시리즈 세번째 관계연산자
--		AND와 BETWEEN의 차이! -> BETWEEN은 이상 이하만 표현 가능
...
-- WHERE 조건식 끝냈고
-- 다음으로는 GROUP BY : 그룹으로 묶어서 연산해서 하나의 결과를 나타내는 것
...
select s_name, s_price
	from JUN03_SNACK
	where s_price < 			(
								select avg(s_price) -- 부속질의(subquery)
								from JUN03_SNACK	-- 부속질의(subquery)
								);
...
-- 과자들 중 최고가?
-- 제일 비싼 과자 이름, 제조사, 가격
-- 제일 싼 과자는 어디에서 만드나요 ? // 중복값이 나온다면 distinct 넣기 !
-- 평균가보다 비싼 과자는 몇 종류 ?
...
create table company(
	...
	c_employee number(5) not null
	constraint fk_company foreign key(c_name)
		references jun03_snack(s_maker)
		delete on cascade
);

insert into company values('크라운', '서울 강남구 서초동', 300)
insert into company values('농심', '서울 강남구 방배동', 200)
insert into company values('롯데', '서울 강남구 대치동', 150)
insert into company values('오리온', '서울 강남구 일원동', 100)
-----------------------강사님 답안--------------------------------------------
...
insert into jun04_maker values('크라운', '서울', 300)
insert into jun04_maker values('농심', '인천', 200)
insert into jun04_maker values('롯데', '부산', 150)
insert into jun04_maker values('오리온', '제주', 100)
...
-- = 쓰면 단일값 이상의 결과를 내보낼 수가 없어서 'single-row ~' 발생 (과자테이블에 동일값 2개)
--		=> 관계연산자 << in >> 사용 // = 자리에 in 사용
select m_address 
	from jun04_maker
	where m_name in (
		select s_maker 
...
-- 인천에 있는 회사에서 만드는 과자의 평균가
select avg(s_price) from JUN03_SNACK
	
select m_name from jun04_maker where m_address = '인천';
