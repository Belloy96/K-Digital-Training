...
create table drink (
	d_name varchar(10 char) primary key,
	d_price number(4) not null,
	d_cafename varchar(10 char) not null
);

create table cafe (
	c_name varchar(10 char) primary key,
	c_address varchar(10 char) not null,
	c_seat number(3) not null
);

insert into drink values ('아메리카노', 2000, 'A');
insert into drink values ('라떼', 2000, 'A');
insert into drink values ('녹차', 2000, 'A');
...
select c_address from cafe where c_address = '서울';

select d_name from drink;
select c_seat from cafe where c_seat >= 90;
