...
public class M {
	...
	//			attribute(cc)
	//		갈색이 닿으면 값을 사용 가능
	//		새로운 요청이 일어나는 상황에서는 값 사용 불가(Second와 ThirdBaby에 a, b, c 값이 안나옴)
	//		Third까지 살아서 값을 보낼 수 있는 방법이 필요함(현재 Third쪽으로는 위의 요청이 닿지 않음)
	//			: session
	
	// 		- parameter
	//			GET : 주소에(컨트롤러?파라미터명=값&파라미터명=값&...)
	//			POST : 내부전달(주소에 컨트롤러?파라미터명=값 안보임) post로 받아올 땐 Encoding 방식도!
	...
	//			값 읽기
	//			Java : request.getParmeter("파라미터명");
	//			JSP(EL) : ${param.파라미터명 }
...
/////////////////////////////////////////////////////////////////////////
	...
	//			=> ID 저장하기, 쇼핑몰 장바구니, 팝업창 (오늘 하루 보지 않기), ...
	...
	//		쿠키보다는 보안이 좋음, 메모리 사용량이 큼(비교적 쿠키보다!)
	...
	//		세션 : 만료되는 시간을 지정할 수는 있지만, 브라우저가 종료되면 바로 삭제
	...
	public static void makeSecondData(HttpServletRequest request, HttpServletResponse response) {
		...
		int d = 40000;
		HttpSession hs = request.getSession(); // 사용자와 서버 간의 연결 상태(세션)
		hs.setMaxInactiveInterval(15);			// session의 유지 시간을 15초
	...
	public static void printSecondDataNew(HttpServletRequest request) {
		...
		// 현재 사용자 컴퓨터에 있는 쿠키들...
		Cookie[] cks = request.getCookies(); // 저장되어있는 쿠키'들'이라서 배열로 가져옴
		for (Cookie ck : cks) {
