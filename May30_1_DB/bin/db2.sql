...
-- 테이블의 기본적인 구조
create table 테이블명(
	컬럼명 자료형 [옵션], -- 옵션 : 필요하면 넣는거! = 제약조건 // [] 넣는거 아님!
	컬럼명 자료형,
	...
);
-- 자료형 (데이터 타입)
...
--			처리 속도가 빠르다 !(용량을 조절할 필요가 없어서)
--			글자 수가 정해져 있다면 char 활용 O
--				ex) 성별, 주민번호, 핸드폰번호, ...
...
--			만약 P에 명시한 것보다 큰 숫자 값을 입력하면 오류가 발생
...
--		값이 123.54인데
--		number(3)		=> 124 (소수점에서 반올림함)
--		number(3, 2) 	=> 오류 ! (p가 3이고, 유효숫자가 5자리여서)
--		number(5, 2) 	=> 123.54 xxx.xx
--		number(7, 1) 	=> 123.5 (s가 1이고, 소수점 둘째자리에서 반올림) xxx.x
--		number(7, -1) 	=> 120 (s가 -1이고, 소수점 왼쪽 첫자리 3이 반올림) xxxxxxx.

--		FLOAT(P) : NUMBER의 하위 타입 + 이진수 기준
...
-- 5 x 0.3 = 약 1.5가 됨
-- float(5)는 실제로 float(2) (1.5를 반올림해서 2가 되었음)
