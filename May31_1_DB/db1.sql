...
create table example5(
	...
	constraint PK_example5 primary key(co1, co2)
);

-- *** 진짜 어려움
-- 4. Foreign Key (외래 키) : 다른 테이블의 특정 컬럼을 조회해서
--							동일한 데이터가 있는 경우에만 입력이 허용됨
--		참조하는 테이블은 참조받는 테이블의 PK(Primary Key)나 Unique로 지정된 컬럼만을 
--			FK(Foreign Key)로 지정할 수 있음
--		외래키 설정을 위해서는 참조받는 컬럼이 먼저 생성되어야 하고,
--		외래키를 심을 테이블이 그 이후에 생성
...
create table 테이블명(
	...
	constraint FK(제약조건)명 foreign key(컬럼명)
		...
		on delete set null || on delete cascade -- (둘 중에 하나 고르기)
		-- 참조받는 테이블(상위 테이블)에서 행 부분을 지웠을 때
		-- 이 테이블(하위 테이블)에 연결되어 있는 행에 대해서
		-- NULL값을 먹일지 or 그 행도 같이 지울지에 대한 옵션
);
...
insert into EXAMPLE7 values('10'); -- example대소문자상관x -> 자동완성이 될 때도 있고..아닐때도.. 
insert into EXAMPLE7 values('20');
insert into EXAMPLE7 values('30');
insert into EXAMPLE7 values('40'); -- 상위 테이블에서 40의 값이 없으므로 에러가 발생
select * from example7;

-- xe6_id가 '10'인 컬럼을 지우면 어떻게 될까 ?
...
--	학생 테이블 : 학생번호(3자리 숫자), 학생 이름, 
--		학생이 소속되어있는 학과의 코드

-- 어떤 특정학과가 없어지면 이 학생의 정보도 지워지게
create table department(
	dm_code number(4) primary key,
	dm_name varchar2(10 char)
);

insert into department values(1111, 'Java학과')
insert into department values(2222, 'Python학과')
insert into department values(3333, 'c학과')
insert into department values(4444, 'c++학과')
select * from department;

create table student(
	st_num number(3),
	st_name varchar2(10 char),
	st_code number(4),
	constraint fk_student foreign key(st_code)
		references department(dm_code)
		on delete cascade
);

insert into student values(111, '김길동', 1111);
insert into student values(222, '나길동', 2222);
insert into student values(333, '박길동', 3333);
insert into student values(444, '이길동', 4444);
select * from student;

delete from department where dm_code = 4444;
---------------------------------------------------------
-- 강사님 답안

-- db2를 위해서 db1에 있던 테이블 삭제
...
-- 내가 쓴 답안에서 student라는 테이블명을 썼기에 student2로 설정
create table student2(
	s_no number(3) primary key,
	s_name varchar2(30 char) not null,
	s_code number(4) not null,
	constraint fk_student2 foreign key(s_code)
		references major(m_code)
		on delete cascade
);

insert into student2 values(101, '곽두팔', 1001);
insert into student2 values(102, '김춘삼', 2001);
insert into student2 values(103, '김비버', 1001);
insert into student2 values(104, '박비버', 2001);
insert into student2 values(105, '최비버', 3001); -- major 테이블에는 3001 코드 X

select * from student2;

delete from major where m_code = 1001;
-- major 테이블에서 1001코드를 지우면, student테이블에서 1001코드 가지고 있던
-- 	모든 학생의 정보가 지워짐
------------------------------------------------------------------------------
