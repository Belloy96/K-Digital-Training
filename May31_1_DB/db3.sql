...
-- 테이블 내에 값이 있는 상태에서 데이터 타입을 변경하면 에러!
...
-- 주의사항 : 데이터타입을 변경할 때는 해당 컬럼의 값을 모두 지워야 변경이 가능
--		그렇지 않으면 오류가 발생!
...
-- 테이블 삭제 시 (휴지통에 넣기)
drop table [테이블명]; -- 외래키 등 제약조건이 걸려있으면 실행이 안됨
drop table [테이블명] cascade constraint; -- 외래키가 관련되어있으면 cascade constraint사용해야함
